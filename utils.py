...
class DAVIS(data.Dataset):
    ...
    def __getitem__(self, index):
       ...
       info = {'name': video, 'num_frames': self.num_frames[video]}
       num_objects = self.num_objects[video] if self.MO else 1
       ...
       for f in range(self.num_frames[video]):
          ...
          raw_masks[f] = raw_mask if self.MO else (raw_mask != 0).astype(np.uint8)
